...
// are not zero-based. Put both these numbers in order in an array and return the array
...
public class TwoSum {
    ...
    public ArrayList<Integer> twoSum(final List<Integer> A, int B) {
        ...
        for(int i = 0; i < A.size(); i++) {
            ...
            int number = A.get(i);
            int match = B - number;
            if(visitedElements.containsKey(match)) {
                result.add(visitedElements.get(match));
                result.add(oneBasedIndex);
                return result;
            } else {
                //put if absent is important, since key may already present.
                visitedElements.putIfAbsent(number, oneBasedIndex);
            }
        }
        return result;
    }
